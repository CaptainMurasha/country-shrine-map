 ...
 */
export function isEnds(pageNumber, pageNumbers, totalCount) {
  if (pageNumber === pageNumbers[0] && pageNumbers[0] !== 1) {
    // 連番配列の先頭が1以外のとき、先頭だけPREVにする
    return PREV;
  } else if (pageNumber === pageNumbers.slice(-1)[0] && pageNumbers[PAGE_RANGE] < getTotalPage(totalCount) - PAGE_RANGE) {
    return NEXT;
  } else {
    return pageNumber;
  }
